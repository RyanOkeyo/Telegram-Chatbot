...
def get_buttons(data: dict, type: str) -> list:
  ...
  if type == "reply_button":
    ...
    for button in data.get("buttons"):
      ...
      button_data = [
        InlineKeyboardButton(button_title, callback_data=button_id)
      ]
...
async def send_medias(update: Update, context: CallbackContext, media: dict,
                      type: str):
  ...
  for file in media:
    ...
    if type == "images":
      ...
      response = requests.get(url)
      await context.bot.send_audi(chat_id=chat_id,
      ...
      response = requests.get(url)
      await context.bot.send_stickers(chat_id=chat_id,
                                      sticker=response.content,
                                      caption=caption)
...
async def respond(message, chat_id, message_type="text") -> dict:
  ...
  """
  response = sarufi.chat(os.environ["sarufi_bot_id"],
                         chat_id,
                         message,
                         channel="whatsapp",
...
async def reply_with_typing(update: Update, context: CallbackContext,
                            message) -> None:
